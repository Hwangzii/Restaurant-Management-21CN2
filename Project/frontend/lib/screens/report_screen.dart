...
class _ReportScreenState extends State<ReportScreen> {
  ...
  void initState() {
    ...
    days =
        getDaysForSelectedDate(selectedDate!); // Get 7 days near selected date
  ...
  Future<void> _fetchRevenueData() async {
    ...
    try {
      ...
      Map<String, double> data = await _reportController
          .fetchRevenueReportForSelectedDate(selectedDate!);
      setState(() {
        revenueData = data;
        // Lấy danh sách ngày tương ứng với dữ liệu
        days = getDaysForSelectedDate(
            selectedDate!); // Cập nhật lại danh sách ngày
      });
    } catch (e) {
      setState(() {
  ...
  List<String> getDaysForSelectedDate(DateTime selectedDate) {
    ...
    for (int i = 0; i < 7; i++) {
      DateTime targetDay =
          selectedDate.add(Duration(days: i)); // Trừ dần từ 0 đến 6
      days.add(DateFormat('dd/MM')
          .format(targetDay)); // Định dạng ngày dưới dạng 'dd/MM'
    }
    return days;
  }
  ...
  Future<void> _pickDate() async {
    ...
    if (picked != null && picked != selectedDate) {
      setState(() {
        selectedDate = picked;
        days = getDaysForSelectedDate(
            picked); // Update days based on selected date
      });
