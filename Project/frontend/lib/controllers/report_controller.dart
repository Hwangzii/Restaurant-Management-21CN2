...
class ReportController {
  ...
  String formatCreatedAt(String createdAt) {
    try {
      ...
      return DateFormat('dd/MM')
          .format(dateTime); // Thay đổi để khớp với định dạng UI
    } catch (e) {
  ...
  Future<Map<String, double>> fetchMonthlyRevenue() async {
    try {
      ...
      List<Map<String, dynamic>> response =
          await _apiService.fetchInvoiceData();
      ...
      for (var invoice in response) {
        try {
          ...
          String formattedMonth =
              DateFormat('MM/yyyy').format(dateTime); // Định dạng tháng/năm

          revenueByMonth[formattedMonth] =
              (revenueByMonth[formattedMonth] ?? 0) + totalAmount;
        } catch (e) {
  ...
  Future<Map<String, double>> fetchRevenueReportForSelectedDate(
    ...
    try {
      ...
      for (var invoice in response) {
        try {
          ...
          revenueByDate[formattedDate] =
              (revenueByDate[formattedDate] ?? 0) + totalAmount;
        } catch (e) {
          print('Lỗi khi xử lý hóa đơn: $e, invoice: $invoice');
        }
      }
      ...
      for (int i = 0; i < 7; i++) {
        DateTime targetDay = selectedDate.add(Duration(days: i));
        String formattedDate =
            DateFormat('dd/MM').format(targetDay); // Sử dụng định dạng dd/MM
